...
PROFESSIONAL_EXPERIENCE_VERSION = "professional-experience-v1-risuai-voice"
...
_INSTALLED = False
_CSS_RENDERED = False
_VOICE_CONTROLS_RENDERED = False
...
:root {
    ...
    --mary-panel-soft: rgba(29, 30, 40, 0.78);
...
def _render_css_once() -> None:
    global _CSS_RENDERED
    if _CSS_RENDERED:
        return
...
def _render_voice_controls_once() -> None:
    global _VOICE_CONTROLS_RENDERED
    if _VOICE_CONTROLS_RENDERED:
        return
    ...
    with st.sidebar:
        ...
        st.toggle(
            ...
            help="Lê somente as falas diretas. Pensamentos e ações não são falados.",
        )
        st.caption("A voz usa o mecanismo do navegador e não lê pensamentos.")

    _VOICE_CONTROLS_RENDERED = True
...
def _is_thought_line(line: str) -> bool:
    ...
    return lowered.startswith(("pensamento:", "penso:", "por dentro:"))
...
def separar_resposta_mary(texto: str) -> tuple[list[tuple[str, str]], str]:
    """Separa fala visível e pensamento, no padrão inspirado no RisuAI.

    Apenas blocos explicitamente marcados por itálico, parênteses em linha própria
    ou prefixo de pensamento são classificados como pensamento. O restante é fala
    direta e poderá ser enviado ao TTS.
    """
...
def _patched_chat_input(*args: Any, **kwargs: Any) -> Any:
    ...
    if isinstance(instance, dict):
        st.caption("As falas diretas podem ser ouvidas. Pensamentos aparecem em outra tipografia.")
