...
def load_catalog_overrides() -> dict[str, dict[str, Any]]:
    ...
    for row in obter_registros_aba(SCENARIOS_SHEET):
        scenario_id = _text(row.get("scenario_id"))
        status = (_text(row.get("status")) or "active").lower()
        if not scenario_id or status not in {"active", "ativo", "published", "publicado"}:
            continue
...
def create_runtime_session(
    *, user_id: str, model: str, prompt_version: str, app_version: str,
    client_type: str = "streamlit", runtime_id: str = "story-runtime-v2",
) -> dict[str, Any]:
    now = _now()
    record = {
        "session_id": f"ses_{uuid4().hex}", "user_id": _text(user_id),
        "started_at": now, "last_activity_at": now, "ended_at": "", "updated_at": now,
        "model": _text(model), "prompt_version": _text(prompt_version),
        "app_version": _text(app_version), "status": "active", "end_reason": "",
        "interaction_count": 0, "last_scenario_id": "", "last_scenario_session_id": "",
        "client_type": _text(client_type), "runtime_id": _text(runtime_id),
        "duration_seconds": "", "active": True,
    }
...
def finish_runtime_session(session_id: str, *, reason: str) -> None:
    now = _now()
    atualizar_registro(
        SESSIONS_SHEET, coluna_chave="session_id", valor_chave=_text(session_id),
        alteracoes={"last_activity_at": now, "ended_at": now, "updated_at": now,
                    "status": "ended", "end_reason": _text(reason) or "ended", "active": False},
    )


def _story_payload(engine_session: StorySession) -> dict[str, Any]:
    return {
...
def create_story_session(
    *, engine_session: StorySession, user_id: str, scenario_version: int, opening_sent: bool,
) -> dict[str, Any]:
...
def persist_story_session(
    *, instance: dict[str, Any], engine_session: StorySession,
    route: str = "", interaction_happened: bool = False,
) -> dict[str, Any]:
    interaction_count = max(0, _int(instance.get("interaction_count"), 0))
    instance.update({
        "status": engine_session.status,
        "interaction_count": interaction_count,
        "current_phase": engine_session.chapter_id,
        "current_route": _text(route),
        "current_beat": engine_session.current_beat,
        "ending_reason": engine_session.ending_reason,
        "ending_sent": not engine_session.is_active,
        "ending_ready": not engine_session.is_active,
...
def persist_interaction(
    *, runtime_session_id: str, user_id: str, scenario_instance: dict[str, Any],
    user_text: str, mary_response: str, model: str, prompt_version: str,
    app_version: str, response_time_ms: int, scenario_beat: str,
    scenario_route: str, scenario_status: str, scenario_completed: bool,
    error: str = "", error_type: str = "", error_stage: str = "", retry_count: int = 0,
) -> dict[str, Any]:
    now = _now()
    ...
    record = {
        "interaction_id": f"int_{uuid4().hex}", "session_id": _text(runtime_session_id),
        "user_id": _text(user_id), "timestamp": now, "updated_at": now,
        "user_text": str(user_text or ""), "mary_response": str(mary_response or ""),
        "model": _text(model), "prompt_version": _text(prompt_version),
        "app_version": _text(app_version), "response_time_ms": max(0, int(response_time_ms or 0)),
        "image_sent": False, "image_width": "", "image_height": "",
        "image_size_bytes": "", "image_mime_type": "", "mary_asked_name": False,
        "error": _text(error), "error_type": _text(error_type), "error_stage": _text(error_stage),
        ...
        "scenario_route": _text(scenario_route), "scenario_beat": _text(scenario_beat),
        "scenario_status": _text(scenario_status), "interaction_number": interaction_number,
        "scenario_completed": bool(scenario_completed), "interaction_key": interaction_key,
    }
    existing = buscar_registro(INTERACTIONS_SHEET, coluna="interaction_key", valor=interaction_key)
    if existing is None:
        adicionar_registro(INTERACTIONS_SHEET, record)
    else:
        atualizar_registro(
            INTERACTIONS_SHEET, coluna_chave="interaction_key",
            valor_chave=interaction_key, alteracoes=record,
        )
    ...
    atualizar_registro(
        SESSIONS_SHEET, coluna_chave="session_id", valor_chave=_text(runtime_session_id),
        alteracoes={"last_activity_at": now, "updated_at": now, "interaction_count": total,
                    "last_scenario_id": record["scenario_id"],
                    "last_scenario_session_id": scenario_session_id, "active": True},
    )
    return record
